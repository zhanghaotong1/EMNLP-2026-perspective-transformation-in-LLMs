...
import numpy as np
import torch
from transformers import AutoModelForSequenceClassification, AutoTokenizer, AutoConfig, Trainer, TrainingArguments, BitsAndBytesConfig, DataCollatorWithPadding
import datasets
import evaluate
...
parser.add_argument('--ckpt', type=str, default='../model/scalability', help='path to store checkpoint')
parser.add_argument('--pred', type=str, default='../data/pred/scalability', help='directory path to store predictions')
parser.add_argument('--task', type=str, default='train', help='train or test')

parser.add_argument('-s', '--size', type=str, required=True, help='qwen model size')
...
def model_name(size):
    if size == '0.8':
        ...
        raise ValueError('Please choose a size from 0.8, 4 or 27!')
    return model_name


def data_process(data):
    if args.description == 'route' and args.question == 'survey':
    ...
    text = data[args.description] + additional_info + ' ' + data['question']
...
tokenizer = AutoTokenizer.from_pretrained(model_name(args.size), use_fast=True)
...
config = AutoConfig.from_pretrained(model_name(args.size), num_labels=4 if args.question == 'route' else 8,
                                    text_config={'num_labels': 4 if args.question == 'route' else 8,
                                                 'id2label': id2label})
base_model = AutoModelForSequenceClassification.from_pretrained(model_name(args.size), low_cpu_mem_usage=True,
...
train_config = TrainingArguments(
    output_dir=f'{args.ckpt}/t{args.template}_{args.description}_{args.question}/{args.size}',
    eval_strategy='epoch',
...
if args.task == 'train':
    ...
    ckpt_dir = f'{args.ckpt}/t{args.template}_{args.description}_{args.question}/{args.size}'
    peft_model = PeftModel.from_pretrained(base_model, f'{ckpt_dir}/{os.listdir(ckpt_dir)[0]}', is_trainable=False)
    peft_model.eval()
    ...
    with open(f'{args.pred}/template{args.template}_{args.description}_{args.question}_{args.size}.jsonl', 'w') as f:
