...
parser.add_argument('--pred', type=str, default='../data/pred/generalisation1', help='directory path to store predictions')
...
parser.add_argument('-t1', '--ft_template', type=str, required=True, help='finetune template')
parser.add_argument('-t2', '--test_template', type=str, required=True, help='test template')
...
train_dataset = datasets.load_dataset('json', data_files=f'{args.data}/template{args.ft_template}_{args.question}_train.jsonl', split=datasets.Split.TRAIN)
train_dataset = train_dataset.map(data_process, batched=False, remove_columns=train_dataset.column_names)
test_dataset = datasets.load_dataset('json', data_files=f'{args.data}/template{args.test_template}_{args.question}_test.jsonl', split=datasets.Split.TRAIN)
test_dataset = test_dataset.map(data_process, batched=False, remove_columns=test_dataset.column_names)
data_collator = DataCollatorWithPadding(tokenizer=tokenizer)

train_config = TrainingArguments(
    output_dir=f'{args.ckpt}/t{args.ft_template}_{args.description}_{args.question}/{args.name}_{args.seed}',
    eval_strategy='epoch',
...
ckpt_dir = f'{args.ckpt}/t{args.ft_template}_{args.description}_{args.question}/{args.name}_{args.seed}'
...
f = open(f'{args.data}/template{args.test_template}_{args.question}_test.jsonl')
data = f.readlines()
f.close()

with open(f'{args.pred}/template{args.ft_template}_template{args.test_template}_{args.description}_{args.question}_{args.name}_{args.seed}.jsonl', 'w') as f:
